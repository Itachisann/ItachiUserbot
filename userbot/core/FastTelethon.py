...
class DownloadSender:
    ...
    def __init__(
        self, sender: MTProtoSender, file: TypeLocation,
...
class UploadSender:
    ...
    def __init__(
        self, sender: MTProtoSender, file_id: int, part_count: int,
        big: bool, index: int, stride: int, loop: asyncio.AbstractEventLoop
    ) -> None:
        self.sender = sender
...
class ParallelTransferrer:
    ...
    async def _init_download(
        self, connections: int, file: TypeLocation, part_count: int,
        part_size: int
    ) -> None:
    ...
    async def _create_download_sender(
        self, file: TypeLocation, index: int, part_size: int,
        stride: int, part_count: int
    ) -> DownloadSender:
        return DownloadSender(
            await self._create_sender(), file, index * part_size, part_size,
            stride, part_count
        )
    ...
    async def _create_upload_sender(
        self, file_id: int, part_count: int, big: bool, index: int,
        stride: int
    ) -> UploadSender:
        return UploadSender(
            await self._create_sender(), file_id, part_count, big, index,
    ...
    async def _create_sender(self) -> MTProtoSender:
        ...
        sender = MTProtoSender(
            self.auth_key, self.loop, loggers=self.client._log
        )
    ...
    async def init_upload(
        self, file_id: int, file_size: int,
        part_size_kb: Optional[float] = None,
        connection_count: Optional[int] = None
    ) -> Tuple[int, int, bool]:
        connection_count = (
            connection_count or self._get_connection_count(file_size)
        )
